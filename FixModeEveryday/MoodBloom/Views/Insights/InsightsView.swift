    ...
    private var moodTrendChart: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if filteredMoodData.isEmpty {
                ...
                SimpleMoodChart(data: filteredMoodData)
                    .frame(height: 200)
                    .padding()
            ...
            ZStack {
                ...
                VStack {
                    ForEach(1..<6) { level in
                        HStack {
                            Text(MoodLevel(rawValue: 6 - level)?.emoji ?? "")
                                .font(.caption)
                ...
                if data.count > 1 {
                    Path { path in
                        ...
                        let stepY = geometry.size.height / 4 // 5 levels, so 4 steps
                        
                        for (index, point) in data.enumerated() {
                            let x = CGFloat(index) * stepX
                            let y = geometry.size.height - (CGFloat(point.moodValue - 1) * stepY)
